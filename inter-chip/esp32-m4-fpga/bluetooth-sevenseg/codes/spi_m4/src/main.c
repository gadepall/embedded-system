...
#define MISC_MAILBOX_REG_0 0x40005110
...
int main(void)
{
    ...
    while(1)
    {
      int digit = *(uint32_t *)(MISC_MAILBOX_REG_0);
      dbg_str("Got ");
      dbg_int(digit);
      dbg_nl();
      display(digit);
...
{
    PyHal_GPIO_SetDir(PIN_A,1);
    PyHal_GPIO_SetDir(PIN_B,1);
    PyHal_GPIO_SetDir(PIN_C,1);
    PyHal_GPIO_SetDir(PIN_D,1);    
    PyHal_GPIO_SetDir(PIN_E,1);
    PyHal_GPIO_SetDir(PIN_F,1);
    PyHal_GPIO_SetDir(PIN_G,1); 
}

void sevenseg(int a, int b, int c, int d, int e, int f, int g)
{
    PyHal_GPIO_Set(PIN_A,a);
    PyHal_GPIO_Set(PIN_B,b);
    PyHal_GPIO_Set(PIN_C,c);
    PyHal_GPIO_Set(PIN_D,d);
    PyHal_GPIO_Set(PIN_E,e);
    PyHal_GPIO_Set(PIN_F,f);
    PyHal_GPIO_Set(PIN_G,g);
}
